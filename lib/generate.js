  ...
  for (var i in dvaTpls) {
    ...
    if (dvaTpls[i].indexOf('[models_name]') !== -1) {
      ...
      if (!fs.existsSync(mockDest)) {
        ...
        const mockTplPath = path.resolve(__dirname, '../page_templates/mock')
        const mockTpls = fs.readdirSync(mockTplPath)
...
function generateMenuAndRouter(config, FolderName, pageName, chineseName) {
  ...
  for (var i in menuAndRouterMap) {
    if (fs.existsSync(menuAndRouterMap[i].path)) {
      let sourceFile = fs.readFileSync(menuAndRouterMap[i].path, 'utf-8')
      //   如果没有自动添加路由和菜单的替换标记 跳出循环 提示开发人员
      if (sourceFile.indexOf(config.replaceFlag) === -1) {
        logger.tips(chalk.blue(`  🤥  文件【${menuAndRouterMap[i].path}】缺少自动创建路由表的标记${config.replaceFlag}，请手动配置`))
        continue
      }
      let destFile = sourceFile.replace(config.replaceFlag, menuAndRouterMap[i].insertItem)
      fs.writeFileSync(menuAndRouterMap[i].path, destFile)
      logger.tips(chalk.blue(`  🎉  成功为文件【${menuAndRouterMap[i].path}】自动添加路由、菜单配置`))

    } else {
      logger.tips(chalk.blue(`  🤥  项目目录缺少文件【${menuAndRouterMap[i].path}】无法自动创建路由表，请手动配置`))
    }
  }
}
